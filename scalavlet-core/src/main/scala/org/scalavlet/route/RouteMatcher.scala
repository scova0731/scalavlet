...
trait RouteMatcher extends RouteTransformer {
  ...
  def apply(route: Route): Route =
    Route.appendMatcher(this)(route)
}
...
final class SinatraRouteMatcher(pattern: String)
  ...
  def apply(requestPath: String) = SinatraPathPatternParser.parse(pattern).genMap(requestPath)
...
final class PathPatternRouteMatcher(pattern: PathPattern)
  ...
  def apply(requestPath: String) = pattern.genMap(requestPath)
...
final class BooleanBlockRouteMatcher(block: => Boolean) extends RouteMatcher {
   ...
   */
  def apply(requestPath: String) = if (block) Some(MultiParams()) else None
