...
import org.apache.http.impl.client.{CloseableHttpClient, HttpClientBuilder, BasicCookieStore}
...
import org.apache.http.entity.mime.{MultipartEntityBuilder, FormBodyPart, MultipartEntity, HttpMultipartMode}
...
case class HttpComponentsClientResponse(res: HttpResponse) extends ClientResponse {
  override lazy val bodyBytes: Array[Byte] = {
  ...
  override def inputStream = res.getEntity.getContent

  override def statusLine = {
...
//  }
  ...
  override def submit[A](
      ...
      body: Array[Byte] = null)(f: ClientResponse => A): A =
  {
    ...
    val res = HttpComponentsClientResponse(client.execute(req))
    f(res)
  }


  override protected def submitMultipart[A](
    method: String,
    path: String,
...
//    }
    ...
    files: Iterable[(String, Any)])
  {
    ...
    req match {
      case r: HttpEntityEnclosingRequestBase =>
        val multipartEntity = MultipartEntityBuilder.create().setMode(HttpMultipartMode.BROWSER_COMPATIBLE)
        params.foreach {
          case (name, value) =>
            multipartEntity.addPart(name, new StringBody(value, ContentType.TEXT_PLAIN))
        }
        ...
        r.setEntity(multipartEntity.build())
  ...
  private def createBody(name: String, content: Any) = content match {
